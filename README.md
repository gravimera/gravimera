...
Gen3D smoke validation checks constrained-joint motion across authored attachment animation channels (for example `idle`, `move`, and `attack_primary`) and reports channel-scoped issues when joint motion is off-axis, exceeds limits, translates despite constraints, stays abnormally biased away from neutral, or when `attack_primary` increases self-intersection relative to the idle pose (generic OBB/SAT).
