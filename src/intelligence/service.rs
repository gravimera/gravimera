...
pub fn run_intelligence_service_blocking(bind: &str, token: Option<String>) -> Result<(), String> {
    let server = match tiny_http::Server::http(bind) {
        Ok(v) => v,
        Err(err) => return Err(format!("Failed to bind {bind}: {err}")),
    };
    ...
    serve_blocking(server, token);
    Ok(())
}

pub(crate) struct EmbeddedIntelligenceService {
    listen_addr: SocketAddr,
    stop_flag: Arc<AtomicBool>,
    thread: Option<JoinHandle<()>>,
}

impl EmbeddedIntelligenceService {
    pub(crate) fn start(bind: &str, token: Option<String>) -> Result<Self, String> {
        let server = match tiny_http::Server::http(bind) {
            Ok(v) => v,
            Err(err) => return Err(format!("Failed to bind {bind}: {err}")),
        };
        ...
        let listen_addr = server.server_addr().to_ip().ok_or_else(|| {
            format!(
                "Embedded intelligence service: unsupported listen addr: {}",
                server.server_addr()
            )
        })?;
...
impl Drop for EmbeddedIntelligenceService {
    fn drop(&mut self) {
...
fn serve_blocking(server: tiny_http::Server, token: Option<String>) {
    let mut state = ServiceState::default();
    for request in server.incoming_requests() {
        handle_http_request(request, token.as_deref(), &mut state);
    }
}

fn serve_until_stop(server: tiny_http::Server, token: Option<String>, stop: Arc<AtomicBool>) {
            ...
            let req: LoadModuleRequest = match serde_json::from_slice(&body) {
                ...
                Err(err) => {
                    respond_json(
                        request,
                        400,
                        serde_json::to_string(&ErrorResponse::new(format!("Invalid JSON: {err}")))
                            .unwrap(),
                    );
                    return;
                }
