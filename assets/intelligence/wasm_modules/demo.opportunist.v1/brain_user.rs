// Demo brain module: demo.opportunist.v1
//
// Guest ABI: `docs/intelligence_wasm_brains.md`
//
// Behavior (simplified v1):
// - Mostly rests, sometimes wanders.
// - When attacked: targets the nearest hostile unit.
// - May opportunistically attack a nearby moving hostile unit.
...
static mut MODE: u32 = 0; // 0=rest, 1=wander
...
pub unsafe extern "C" fn brain_tick_v1(
    ...
    let notice_m = if is_ranged { 12.0 } else { 7.0 };
    let moving_threshold_mps = 0.35f32;
    let forget_target_ticks = 600u64;
    let attack_chance = 0.45f32;

    let mut rng = SplitMix64::new(obs.rng_seed ^ 0x0F70_12A9_84B1_4C5E);
    ...
    // If attacked and no target, pick nearest hostile.
    if attacked && HAS_TARGET == 0 {
    ...
    // If we have a target, attack it (and chase if melee).
    if HAS_TARGET != 0 {
        let mut count = 0u32;
    ...
    // No target: maybe pick a moving hostile unit to attack.
    let max2 = notice_m * notice_m;
    let mut candidate: Option<NearbyEntityView> = None;
    ...
    if candidate.is_some() && rng.next_f32() < attack_chance {
        let e = candidate.unwrap();
        HAS_TARGET = 1;
        TARGET_ID_LO = e.entity_id_lo;
        TARGET_ID_HI = e.entity_id_hi;
        TARGET_POS = add_pos(self_pos, e.rel_pos);
        TARGET_LAST_SEEN_TICK = tick;
    ...
    // Rest/wander loop (~3/4 rest ratio).
    if tick >= MODE_UNTIL_TICK {
        let r = rng.next_f32();
        if r < 0.75 {
            MODE = 0;
            MODE_UNTIL_TICK = tick.wrapping_add(rng.gen_range_u32(180, 360) as u64);
            HAS_WANDER_TARGET = 0;
        } else {
            MODE = 1;
            MODE_UNTIL_TICK = tick.wrapping_add(rng.gen_range_u32(120, 240) as u64);
            HAS_WANDER_TARGET = 1;
            WANDER_TARGET = random_wander_target(self_pos, 7.0, &mut rng);
        }
    }
...
impl<'a> Obs<'a> {
    ...
    fn nearby(&self, index: usize) -> Option<NearbyEntityView> {
        ...
        let rel_vel = [
            read_f32(self.bytes, base + 44),
            read_f32(self.bytes, base + 48),
            read_f32(self.bytes, base + 52),
        ];
