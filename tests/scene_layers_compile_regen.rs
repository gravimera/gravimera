...
#[test]
fn compile_is_deterministic_and_regeneration_is_scoped() {
    let listener = match TcpListener::bind("127.0.0.1:0") {
        Ok(v) => v,
        Err(err) if err.kind() == std::io::ErrorKind::PermissionDenied => {
            eprintln!("Skipping scene layers test: bind not permitted ({err}).");
            return;
        }
    ...
    let temp_root = make_temp_dir("gravimera_scene_layers_compile");
    ...
    let layer_a_path = temp_src.join("layers/layer_a.json");
    let updated_layer_a = r#"{
  "format_version": 1,
  "instances": [
    {
      "local_id": "a1",
...
"#;
    std::fs::write(&layer_a_path, updated_layer_a).expect("write updated layer a");
    ...
    let regen_body = serde_json::json!({ "layer_id": "layer_a" }).to_string();
    ...
    assert_eq!(regen_json["despawned"].as_u64().unwrap_or(0), 1);

    let sig3 = get_signature(addr);

    // Pinned content and other layers must remain unchanged.
    assert_eq!(sig1["pinned_sig"], sig3["pinned_sig"]);
    assert_eq!(sig1["layer_sigs"]["layer_b"], sig3["layer_sigs"]["layer_b"]);

    // The regenerated layer must change (we moved a1 and removed a2).
    assert_ne!(sig1["layer_sigs"]["layer_a"], sig3["layer_sigs"]["layer_a"]);
    assert_ne!(sig1["overall_sig"], sig3["overall_sig"]);
