...
- Terrain panel preview/apply: click a terrain item to open its preview, then click **Apply** to switch the scene terrain. The preview overlay also offers **Edit** (opens GenFloor to overwrite this terrain), **Delete**, and **Exit** (Default Terrain only shows **Apply**/**Exit**).
