...
- [ ] (2026-03-04) Add protocol types (JSON) and budget limits with unit tests.
- [ ] (2026-03-04) Implement `gravimera_intelligence_service` sidecar with demo brain module(s) and `/v1/*` endpoints.
- [ ] (2026-03-04) Implement host client + Bevy plugin to tick brains and apply outputs as `MoveOrder` (host-authoritative).
- [ ] (2026-03-04) Add an automation-friendly way to attach/detach brains (config + optional Automation API endpoint) and a regression test.
- [ ] (2026-03-04) Run `cargo test` + rendered smoke test; record key transcripts here.
...
There is no standalone brain / intelligence service implementation yet.
...
(Add short transcripts here as milestones complete, e.g. `cargo test` and smoke test output summaries.)

## Interfaces and Dependencies

### New Rust modules (planned)

- `crate::intelligence::protocol`: serde types for `TickInput`, `TickOutput`, lifecycle requests/replies, errors, and budget caps.
- `crate::intelligence::sidecar`: host-side HTTP client for the intelligence service (local sidecar).
- `crate::intelligence::host_plugin`: Bevy plugin that produces tick inputs and applies outputs as host-authoritative commands.

### New binaries (planned)
