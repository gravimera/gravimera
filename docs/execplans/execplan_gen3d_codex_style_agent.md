...
- `get_tool_detail_v1({tool_id:"..."})` returns a detailed tool descriptor including `args_schema` and `args_example` for that id (call multiple times for multiple tools).
...
- The AI can call `get_tool_detail_v1` successfully and those calls are logged.
...
- `crate::gen3d::agent::tools` must define:
  - a registry that can answer `get_tool_detail_v1`
