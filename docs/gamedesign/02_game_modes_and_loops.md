...
In Build mode, the top toolbar includes **Scenes**, **Prefabs**, and **Floors** toggles plus a Play/Build toggle. Selecting **Prefabs** shows a left-side Prefabs panel listing **realm prefab packages** for the active realm. The panel includes a search box; each row shows a thumbnail + short name (fallback to prefab id for legacy prefabs). The list sorts by last-modified time by default, and query results are ranked by relevance. Clicking a row selects it (visually marked in the list) and opens a preview overlay (↑/↓ cycles through matching items and auto-scrolls the list to keep the selection visible); dragging a row spawns the prefab into the world (closing any open preview overlay). Long lists are scrollable via a vertical scrollbar with a draggable thumb, and while the cursor is over scrollable UI panels such as **Scenes**, **Prefabs**, and **Floors**, mouse-wheel scrolling stays captured by that UI instead of zooming the world camera. The preview overlay uses a fixed-size 3D viewport with orbit controls (rotate: LMB drag, zoom: mouse wheel) and a scrollable info box; while the cursor is over the overlay, world inputs (camera/selection/commands) are captured by the preview UI. Selecting **Scenes** shows a left-side Scenes panel listing all scenes in the current realm, with an **Add Scene** flow that creates a new scene workspace and switches to it (saving the current scene first). Selecting **Floors** shows a left-side Floors panel for browsing, previewing, importing, exporting, and applying realm floors.
