...
In Build mode, the top toolbar includes **Scenes** and **Models** toggles, a **Scene Builder** button, and a Play/Build toggle. Selecting **Models** shows a left-side Models panel listing depot models; long lists are scrollable via a vertical scrollbar with a draggable thumb. Selecting **Scenes** shows a left-side Scenes panel listing all scenes in the current realm, with an **Add Scene** flow that creates a new scene workspace and switches to it (saving the current scene first). Clicking **Scene Builder** opens the Scene Builder panel.
